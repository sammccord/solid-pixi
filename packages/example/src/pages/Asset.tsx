  ...
  return (
    <Application background="#1099bb">
      <RotatingHalvedContainer>
        <AsyncAsset<Texture, OtherProps> component={Bunnies} urls={'/bunny.png'} foo="bar" />
      </RotatingHalvedContainer>
    </Application>
  )
}
...
function Bunnies(props: { texture: Texture } & OtherProps) {
  console.log('!!', props.foo, props.texture)
  return (
    <For each={Array.from({ length: 25 })}>
      {(_, i) => <Sprite from={props.texture} x={(i() % 5) * 40} y={Math.floor(i() / 5) * 40} />}
    </For>
  )
}
