...
export type ExtendedAnimatedSprite<T extends Record<string, any>> =
  pxAnimatedSprite & T;
...
export function AnimatedSprite<T extends Record<string, any>>(
  ...
  if (pixis.textures) {
    sprite = new pxAnimatedSprite(
      pixis.textures,
      props.autoUpdate
    ) as ExtendedAnimatedSprite<T>;
  } else if (ours.fromFrames) {
    sprite = pxAnimatedSprite.fromFrames(
      ours.fromFrames
    ) as ExtendedAnimatedSprite<T>;
  } else if (ours.fromImages) {
    sprite = pxAnimatedSprite.fromImages(
      ours.fromImages
    ) as ExtendedAnimatedSprite<T>;
  } else {
