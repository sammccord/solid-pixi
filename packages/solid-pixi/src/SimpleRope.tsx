import { DisplayObjectEvents, IPoint, Resource, SimpleRope as pxSimpleRope, Texture } from 'pixi.js'
import { createEffect, JSX, onCleanup, splitProps } from 'solid-js'
import { Events, EventTypes } from './events'
import { CommonPropKeys, CommonProps, Transform } from './interfaces'
import { ParentContext, useParent } from './ParentContext'

export type ExtendedSimpleRope<T extends Record<string, any>> = pxSimpleRope & T
...
export function SimpleRope<T extends Record<string, any>>(props: SimpleRopeProps<T>): JSX.Element {
  const [ours, events, pixis] = splitProps(props, CommonPropKeys, EventTypes)

  let rope =
    ours.as ||
    (new pxSimpleRope(pixis.texture, pixis.points, pixis.textureScale) as ExtendedSimpleRope<T>)
  ...
  return <ParentContext.Provider value={rope}>{ours.children}</ParentContext.Provider>
}
